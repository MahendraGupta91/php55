...
if (!isset($ArticleFlag)) 
{
    ...
    if ($conn->connect_error) 
        ...
        $aa='abc';
    /* @var $_toString type */
    $ArticleName=$_POST["topic1"]."_".$_POST["topic2"].$_SESSION["AuthorId"];
    $Topic1=filter_input(INPUT_POST,"topic1");
    $Topic2=filter_input(INPUT_POST, "topic2");
        $sql = "INSERT INTO article (Article_Name, AuthorId, LastEdited, topic1, topic2)
            VALUES ('$ArticleName', $AuthorId, $TodayDate,'$Topic1','$Topic2')";
    ...
    /* @var $result type */
     ...
     //$sql2="CREATE TABLE '$Articlename'(sn int primary key AUTO_INCREMENT,topic1 varchar, topic2 varchar))";      
      $sql = "CREATE TABLE $ArticleName (sn INT(2) UNSIGNED AUTO_INCREMENT PRIMARY KEY, topic1 TEXT,
          topic2 TEXT, imgname1 VARCHAR(15), imgname2 VARCHAR(15),imgtype1 VARCHAR(5), imgtype2 VARCHAR(5))";           
            if ($conn->query($sql) === TRUE) 
            {
                echo " ";
            } 
            ...
            echo " kkkkk".($_FILES["image1"]&& $_FILES["image1"]["error"])."LLLL";
        ...
        echo "Error:33333333333333333333333 " . $_FILES["image1"]["error"];
    }
}
